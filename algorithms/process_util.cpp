...
{
    if (image.empty()) {
        qDebug() << "getImageEdges: input image is empty!";
        return cv::Mat();
    }

    qDebug() << "getImageEdges: input" << image.cols << "x" << image.rows;

    // ========== 1. 手动转换为 8位图像 ==========
    cv::Mat image_8u(image.rows, image.cols, CV_8U);

    if (image.type() == CV_16U) {
        const uint16_t* src = reinterpret_cast<const uint16_t*>(image.data);
        uint8_t* dst = image_8u.data;
        ...
        for (size_t i = 0; i < total; i++) {
            int val = static_cast<int>((src[i] - minVal) * scale);
    ...
    // ========== 2. 手动 Sobel 边缘检测 ==========
    cv::Mat result(image.rows, image.cols, CV_8U, cv::Scalar(0));

    // 处理内部像素（排除边界）
    for (int y = 1; y < image.rows - 1; y++) {
        const uint8_t* row0 = image_8u.ptr<uint8_t>(y - 1);
        const uint8_t* row1 = image_8u.ptr<uint8_t>(y);
        const uint8_t* row2 = image_8u.ptr<uint8_t>(y + 1);
        uint8_t* dst = result.ptr<uint8_t>(y);

        for (int x = 1; x < image.cols - 1; x++) {
            // Sobel X 梯度
            int gx = -row0[x - 1] + row0[x + 1] - 2 * row1[x - 1] + 2 * row1[x + 1] - row2[x - 1]
                     + row2[x + 1];

            // Sobel Y 梯度
            int gy = -row0[x - 1] - 2 * row0[x] - row0[x + 1] + row2[x - 1] + 2 * row2[x]
                     + row2[x + 1];

            // 梯度幅值（使用绝对值近似）
            int mag = (abs(gx) + abs(gy)) / 2;
            dst[x] = (mag > 255) ? 255 : mag;
        }
    }
    ...
    if (image.rows > 2) {
        // 第一行复制第二行
        memcpy(result.ptr<uint8_t>(0), result.ptr<uint8_t>(1), image.cols);
        // 最后一行复制倒数第二行
        memcpy(result.ptr<uint8_t>(image.rows - 1), result.ptr<uint8_t>(image.rows - 2), image.cols);
    }

    // 处理左右边界列
    if (image.cols > 2) {
        for (int y = 0; y < image.rows; y++) {
            uint8_t* row = result.ptr<uint8_t>(y);
            row[0] = row[1];
            row[image.cols - 1] = row[image.cols - 2];
        }
    }

    qDebug() << "getImageEdges: output" << result.cols << "x" << result.rows;
    return result;
}
...
std::tuple<cv::Mat, std::string> getBinaryImage(const cv::Mat& image, bool return_main_axis)
{
    ...
    if (image.empty() || image.data == nullptr) {
        qCritical() << "getBinaryImage: invalid input!";
        return std::make_tuple(cv::Mat(), "");
    }

    int H = image.rows;
    int W = image.cols;

    qDebug() << "getBinaryImage: input" << W << "x" << H;

    // ========== 2. 手动创建和初始化二值图像 ==========
    // 使用 new 分配内存，完全绕过 OpenCV 的内存管理
    uchar* binary_data = new uchar[H * W];
    memset(binary_data, 0, H * W); // 手动设置为0

    // 用分配的内存创建 cv::Mat（但后续尽量少用 OpenCV 函数）
    cv::Mat binary_img(H, W, CV_8UC1, binary_data);

    // ========== 3. 峰值检测 ==========
    for (int y = 0; y < H; y++) {
        const uchar* src_row = image.ptr<uchar>(y);
        uchar* dst_row = binary_img.ptr<uchar>(y);
        ...
        for (int x = 2; x < W - 2; x++) {
            ...
            // 计算 prominence
            uchar left_min = 255, right_min = 255;
            for (int dx = 1; dx <= 5; dx++) {
                if (x - dx >= 0)
                    left_min = std::min(left_min, src_row[x - dx]);
                if (x + dx < W)
                    right_min = std::min(right_min, src_row[x + dx]);
            }

            if (val - std::min(left_min, right_min) >= 5) {
                dst_row[x] = 255;
            }
        }
    }
    ...
    // ========== 4. 手动形态学操作 ==========
    uchar* temp_data = new uchar[H * W];
    memset(temp_data, 0, H * W);

    // 膨胀
    for (int y = 1; y < H - 1; y++) {
        for (int x = 1; x < W - 1; x++) {
            if (binary_data[y * W + x] == 255) {
                for (int dy = -1; dy <= 1; dy++) {
                    for (int dx = -1; dx <= 1; dx++) {
                        temp_data[(y + dy) * W + (x + dx)] = 255;
                    }
                }
            }
        }
    ...
    // 腐蚀
    uchar* result_data = new uchar[H * W];
    memset(result_data, 0, H * W);
    ...
    memcpy(result_img.data, result_data, H * W);
    ...
    if (return_main_axis) {
        ...
        int cols_with_pixels = 0;

        // 统计行
        for (int y = 0; y < H; y++) {
            const uchar* row = result_img.ptr<uchar>(y);
            for (int x = 0; x < W; x++) {
                if (row[x] > 0) {
                    rows_with_pixels++;
                    break;
        ...
        double h_ratio = static_cast<double>(rows_with_pixels) / H;
        double w_ratio = static_cast<double>(cols_with_pixels) / W;

        std::string main_axis = (h_ratio > w_ratio) ? "y" : "x";
        return std::make_tuple(result_img, main_axis);
    }

    return std::make_tuple(result_img, "");
}
                                                                      ...
                                                                      int exp_lines)
{
    // ========== 1. 输入检查 ==========
    if (img_otsu.empty()) {
        qCritical() << "fitComponentCurve: input image is empty!";
        return std::make_tuple(StatusCode::EMPTY_CABLE_ERROR, std::vector<CurveFitResult>{});
    }

    if (img_otsu.data == nullptr) {
        qCritical() << "fitComponentCurve: image data is null!";
        return std::make_tuple(StatusCode::EMPTY_CABLE_ERROR, std::vector<CurveFitResult>{});
    }

    qDebug() << "fitComponentCurve: input" << img_otsu.cols << "x" << img_otsu.rows;

    int H = img_otsu.rows;
    int W = img_otsu.cols;

    // ========== 2. 手动实现连通组件分析 ==========
    // 使用并查集(Union-Find)进行连通组件标记
    std::vector<int> parent(H * W);
    for (int i = 0; i < H * W; i++) {
        parent[i] = i;
    }
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            ...
            int idx = y * W + x;
            int root = find(idx);
            component_points[root].push_back(cv::Point(x, y));
        }
    }

    qDebug() << "fitComponentCurve: found" << component_points.size() << "components";

    // ========== 4. 过滤太小的组件 ==========
    std::vector<std::vector<cv::Point>> valid_components;
    int min_component_size = 50; // 最小像素数

    for (const auto& pair : component_points) {
        if (pair.second.size() >= static_cast<size_t>(min_component_size)) {
            valid_components.push_back(pair.second);
        }
    }

    qDebug() << "fitComponentCurve:" << valid_components.size() << "components passed size filter";
    ...
    for (const auto& points : valid_components) {
        double sum_x = 0, sum_y = 0;
        for (const auto& pt : points) {
            sum_x += pt.x;
            sum_y += pt.y;
        }
        centroids.push_back(cv::Point2d(sum_x / points.size(), sum_y / points.size()));
    }

    // ========== 6. 按质心排序 ==========
    std::vector<int> indices(valid_components.size());
    ...
    for (size_t idx = 0; idx < indices.size(); idx++) {
        int comp_idx = indices[idx];
        const auto& points = valid_components[comp_idx];
        ...
        // 检查是否有足够的点
        if (independent.size() < 4) {
            qDebug() << "Component" << idx << "has too few points:" << independent.size();
            continue;
        }
        ...
        try {
            ...
            qDebug() << "Curve fitting failed for component" << idx << ":" << e.what();
            continue;
        }
        ...
        fit.independent_var = (main_axis == "y") ? "y" : "x";
        fit.dependent_var = (main_axis == "y") ? "x" : "y";
        fit.centroid = centroids[comp_idx];

        all_fits.push_back(fit);
    }

    qDebug() << "fitComponentCurve:" << all_fits.size() << "curves fitted successfully";
    ...
    if (all_fits.empty()) {
        qDebug() << "fitComponentCurve: no valid curves found";
        return std::make_tuple(StatusCode::EMPTY_CABLE_ERROR, std::vector<CurveFitResult>{});
    }
...
fitMedianCurve(const cv::Mat& img_otsu,
               ...
               const std::string& main_axis,
               int exp_lines) {
    if (all_fits.empty()) return all_fits;
    ...
    mid_fit.params = {params.at<double>(0, 0), params.at<double>(1, 0),
...
getRadiusByMidPopt(const std::vector<CurveFitResult>& all_fits,
    ...
    for (int i = 0; i < 200; ++i) {
        mid_curve[i] = cubicFunction(common_ind_values[i], 
            mid_params[0], mid_params[1], mid_params[2], mid_params[3]);
        mid_derivatives[i] = cubicDerivative(common_ind_values[i],
            mid_params[0], mid_params[1], mid_params[2]);
    }
    ...
    for (int i = 0; i < sample_count; ++i) {
        ...
        double norm = std::sqrt(normal_vecs_x[i] * normal_vecs_x[i] + normal_vecs_y[i] * normal_vecs_y[i]);
    ...
    for (const auto& fit : all_fits) {
        ...
        for (int s_idx = 0; s_idx < sample_count; ++s_idx) {
            ...
            for (int sp = 0; sp < n_search_points; ++sp) {
                ...
                if (x_t < min_common_ind || x_t > max_common_ind) continue;
                
                double y_curve_t = cubicFunction(x_t, fit_params[0], fit_params[1], 
...
MeasurementData analyze2DWireData(const std::vector<RadiusResult>& left_right_data,
    ...
    inner_center.x = lr_ratio * (-left_right_data[x_indices[1]].avg_radius + 
                                  left_right_data[x_indices[2]].avg_radius) / 2.0;
    inner_center.y = ud_ratio * (-up_down_data[y_indices[1]].avg_radius + 
                                  up_down_data[y_indices[2]].avg_radius) / 2.0;
    ...
    for (int angle = 0; angle < 360; ++angle) {
        ...
        if (discriminant >= 0) {
            ...
            if (t1 > 0) positive_ts.push_back(t1);
            if (t2 > 0) positive_ts.push_back(t2);
    ...
    int min_angle = static_cast<int>(std::min_element(thickness_at_angles.begin(), 
                                                      thickness_at_angles.end()) - thickness_at_angles.begin());
