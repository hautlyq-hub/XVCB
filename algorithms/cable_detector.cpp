...
CableDetector::CableDetector(double sample_ratio, int sample_count,
...
CableDetector::measure(const std::string& lr_dcm_path,
    ...
    try {
        ...
        qDebug() << (e.what());
        return std::make_tuple(e.getCode(), cv::Mat(), cv::Mat(), 
                               cv::Mat(), cv::Mat(), MeasurementData{});
    }
    ...
    try {
        ...
        return std::make_tuple(StatusCode::READ_IMAGE_ERROR, cv::Mat(), cv::Mat(),
                               cv::Mat(), cv::Mat(), MeasurementData{});
    } catch (const std::exception& e) {
        qDebug() << ("Error reading images: " + std::string(e.what()));
        return std::make_tuple(StatusCode::READ_IMAGE_ERROR, cv::Mat(), cv::Mat(),
    ...
    if (onnx_model_ && onnx_model_->isLoaded()) {
        try {
            ...
            return std::make_tuple(StatusCode::DENOISE_ERROR, cv::Mat(), cv::Mat(),
                                   cv::Mat(), cv::Mat(), MeasurementData{});
        }
    }
    ...
    try {
        ...
        // Edge detection
        cv::Mat* lr_img_for_edges = new cv::Mat(lr_img_post_.clone());
        cv::Mat* ud_img_for_edges = new cv::Mat(ud_img_post_.clone());

        cv::Mat lr_edges = getImageEdges(*lr_img_for_edges);
        cv::Mat ud_edges = getImageEdges(*ud_img_for_edges);

        delete lr_img_for_edges;
        delete ud_img_for_edges;
        qDebug() << "Edge detection done, results:" << lr_edges.cols << "x" << lr_edges.rows
                 << "and" << ud_edges.cols << "x" << ud_edges.rows;
        ...
        return std::make_tuple(StatusCode::PROCESS_IMAGE_ERROR, cv::Mat(), cv::Mat(),
                               lr_img_post_, ud_img_post_, MeasurementData{});
    }
    ...
    try {
        ...
        if (status1 != StatusCode::SUCCESS) {
            ...
            return std::make_tuple(status1, cv::Mat(), cv::Mat(),
                                   lr_img_post_, ud_img_post_, MeasurementData{});
        }
        ...
        if (status2 != StatusCode::SUCCESS) {
            ...
            return std::make_tuple(status2, cv::Mat(), cv::Mat(),
                                   lr_img_post_, ud_img_post_, MeasurementData{});
        }
        ...
        auto [lr_profile, lr_results] = getRadiusByMidPopt(
            lr_all_fits, lr_main_axis_, sample_ratio_, sample_count_, lr_img_otsu_);
        auto [ud_profile, ud_results] = getRadiusByMidPopt(
            ud_all_fits, ud_main_axis_, sample_ratio_, sample_count_, ud_img_otsu_);
        ...
        return std::make_tuple(StatusCode::ANORMAL_CABLE_ERROR, cv::Mat(), cv::Mat(),
                               lr_img_post_, ud_img_post_, MeasurementData{});
    }
    ...
    cv::Mat lr_profile_copy = lr_profile_.clone();
    cv::Mat ud_profile_copy = ud_profile_.clone();
    cv::Mat lr_img_copy = lr_img_post_.clone();
    cv::Mat ud_img_copy = ud_img_post_.clone();
    MeasurementData measure_copy = measure_data_; // 确保 MeasurementData 有正确的拷贝构造函数
