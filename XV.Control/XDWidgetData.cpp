...
XDWidgetData::XDWidgetData(XDFramelessHelperPrivate *_d, QWidget *window, QWidget *shadowContainerWidget)
    : d(_d)
    , m_window(window)
    , m_oldShadowWidth(0)
...
QWidget *XDWidgetData::widget()
{
    return m_window;
}

void XDWidgetData::handleWidgetEvent(QEvent *event)
{
    switch (event->type()) {
...
void XDWidgetData::updateRubberBandStatus()
{
...
void XDWidgetData::setShadowWidth(const int width)
{
    if (width > 0)
...
void XDWidgetData::handleMousePressEvent(QMouseEvent *event)
{
...
void XDWidgetData::handleMouseReleaseEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        m_bLeftButtonPressed = false;
        m_bLeftButtonTitlePressed = false;
...
void XDWidgetData::handleMouseMoveEvent(QMouseEvent *event)
{
...
void XDWidgetData::handleLeaveEvent(QMouseEvent *event)
{
    Q_UNUSED(event)
    if (!m_bLeftButtonPressed) {
...
void XDWidgetData::handleHoverMoveEvent(QMouseEvent *event)
{
...
void XDWidgetData::updateCursorShape(const QPoint &gMousePos)
{
...
void XDWidgetData::resizeWidget(const QPoint &gMousePos)
{
    QRect origRect;
...
void XDWidgetData::moveWidget(const QPoint &gMousePos)
{
    if (d->m_bRubberBandOnMove && Qt::WindowMaximized != m_window->windowState()) {
        ...
        if (Qt::WindowMaximized == m_window->windowState()) {
            ...
            XDShadowWindow<QWidget> *pWindow = static_cast<XDShadowWindow<QWidget> *>(m_window);
